# MAP Glue — Master Services Agreement (MSA)

**Template only — not legal advice.** Replace all `[bracketed]` fields. Have a qualified attorney review before use.

This MSA governs Client's use of **MAP Glue** ("**Glue**") and related services. **Specific Mesh systems, Signals, fees, and deliverables** for each engagement are defined in one or more **Statements of Work (SOWs)** signed by both parties. The first Glue SOW is typically [sow-template-glue.md](./sow-template-glue.md).
...
1.4 **Prior trial or discovery work.** Provider's standard Glue offer includes **free discovery, free Mesh/Signal implementation, and a free 2-week evaluation trial** under a separate trial agreement ([glue-trial-terms.md](./glue-trial-terms.md)). If Client completed that trial (or documented discovery) before signing this MSA and an initial Glue SOW, Provider carries forward configuration for the same Mesh and Signals. Client pays **Mesh activation and monthly subscription** on conversion as stated in the SOW; Client does **not** pay a second implementation fee for identical trial scope.
...
- Client access to Glue (Mesh refresh and Signal delivery) ends on the termination effective date (or as otherwise agreed for wind-down).
...
3.6 **Implementation in progress.** If Client terminates during an active Glue SOW after work has begun, Client pays activation fees due under the SOW and any approved change orders; Provider delivers work product completed to date in Provider's standard format.
...
- Use Glue for lawful internal business purposes only
- Not use Glue output for regulatory filings, audited financials, tax positions, or safety-critical decisions without independent verification
- Not reverse-engineer, resell, or sublicense the platform except as expressly permitted
- Provide and maintain a written list of authorized named users (maximum **five (5)** under a standard Glue SOW, unless a SOW or change order states otherwise)
- Accept that Glue delivers **catalog Mesh + Signal templates** — missing or unsupported source fields may result in a Signal marked limited / N/A rather than a custom rebuild
...
5.1 **Client data ownership.** Client retains all right, title, and interest in **Client Data** — data Client provides or that is extracted from Client's systems for Glue.

5.2 **Provider use.** Provider processes Client Data **solely** to deliver the services, maintain and improve Glue (using **aggregated, anonymized** data only — never Client-identifiable content), and comply with law.
...
6.1 **Provider IP.** Provider retains all rights in Glue, including platform software, connectors, entity-resolution and reconciliation logic, Mesh and Signal catalog templates, definition packs, documentation frameworks, and know-how developed before or outside Client-specific configuration ("**Provider IP**").

6.2 **License to Client.** During an active subscription and subject to payment, Provider grants Client a **non-exclusive, non-transferable** license to access Glue for Client's internal business purposes — including viewing and acting on Signals delivered via the channel stated in the SOW. Unless a SOW or change order states otherwise, Client may designate up to **five (5)** named users. Provider provisions access only for users Client names in writing and only within that cap.
...
| `[e.g. Amazon QuickSight / Glue app / email provider]` | Signal / briefing delivery channel | US |
...
8.3 **DISCLAIMER.** EXCEPT AS STATED IN 8.1, GLUE AND SERVICES ARE PROVIDED **"AS IS."** PROVIDER DISCLAIMS ALL OTHER WARRANTIES, EXPRESS OR IMPLIED, INCLUDING MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

8.4 **No professional advice.** Glue is an **operational insight tool**, not investment, legal, tax, accounting, or audit advice. Client is solely responsible for business decisions based on Glue output, including whether to bill, collect, or change operations.

8.5 **Data accuracy.** Signals reflect Client source systems, mappings, catalog definitions, and confidence rules. Provider does not warrant that output is complete, free of false positives/negatives, or suitable for audited financial statements or regulatory compliance without Client's independent verification. Native sync between Client systems (if any) is not replaced by Glue.
...
12.1 **Glue support:** Email support during business hours with **1 business day** target response time for non-critical issues, unless a SOW states otherwise.

12.2 **Availability.** Provider targets Mesh refresh on the cadence stated in the SOW on a best-efforts basis. Scheduled maintenance will be communicated when practicable. Glue is not guaranteed uninterrupted or error-free. Real-time or sub-daily refresh is not included unless a SOW expressly states otherwise.
...
13.7 **Entire agreement.** This MSA, together with executed Glue SOWs and any referenced NDA, is the entire agreement for Glue and supersedes prior discussions on the same subject.
...
The parties' first Glue engagement under this MSA is defined in:

**SOW number:** `[SOW-GLUE-2026-001]`  
**Effective date:** `[Same as or after MSA date]`  
**Document:** Attached `[sow-template-glue.md / PDF export]`

Additional Glue SOWs (add Mesh systems, add catalog Signals, change delivery channel, etc.) may be added by mutual written agreement without amending this MSA.
...
- [ ] Subprocessor table matches actual Glue stack and delivery channel  
- [ ] Support email live  
- [ ] County for venue filled in  
- [ ] NDA cross-reference if separate NDA exists from trial/discovery  
- [ ] Exhibit A references correct Glue SOW number and date  
- [ ] Sign MSA and Glue SOW same day (or MSA first)  
- [ ] W-9 ready if Client requested  
- [ ] E&O insurance status confirmed  
- [ ] Attorney review scheduled before first real customer  
- [ ] Confirm this is a **Glue** deal — pricing per glue-pricing-sheet ($4,000 / $600 flat Mesh)  
...
|---|---|
| [sow-template-glue.md](./sow-template-glue.md) | Sign with this MSA at Glue paid conversion |
| [glue-pricing-sheet.md](./glue-pricing-sheet.md) | Client-facing Mesh flat pricing ($4,000 / $600) |
...
| [glue-trial-terms.md](./glue-trial-terms.md) | Free discovery + implementation + 2-week trial |
...
| Date | Change |
|---|---|
| 2026-07-21 | Initial Glue MSA — Mesh + Signal model; catalog templates; read-only; pairs with sow-template-glue |
| 2026-07-21 | Fees default to flat Mesh (all applicable Signals); link glue-pricing-sheet |
