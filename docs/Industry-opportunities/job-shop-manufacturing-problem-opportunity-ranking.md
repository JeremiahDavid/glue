...
**Purpose:** Pre-research ranking of problems Glue could productize as **single-problem solutions** — before discovery calls. Launch with one Signal; expand as the product matures.
...
- Glue architecture constraints (cross-system joins, confidence gates, no invented dollars)
...
| 4 | Recurring weekly pain; clear ROI story | Mostly standard extracts; light glue |
...
**Glue novelty (1–5):** How much the problem *requires* cross-system reconciliation (vs a single ERP report). Higher = better Signal for Glue as a differentiated product.
...
| Rank | Problem | Imp. | Ease | Glue | Launch | Why it ranks here |
|---|---|---|---|---|---|---|
| **1** | **Shipped / completed but not invoiced** | 5 | 4 | 5 | **18** | Direct cash stuck; classic ops↔finance gap; ERP ship + QB invoice join is Glue's hero proof; industry cases cite 10–15%+ leakage and large $ stuck between dispatch and invoice |
| **2** | **Past-due AR — ranked collections queue** | 5 | 5 | 2 | **17** | Cash urgency max; easiest data (mostly QB); **weak Glue novelty** — Intuit and bookkeepers already compete; better as *add-on* to #1 than solo launch |
...
### Tier B — Strong follow-on products (same glue spine)

| Rank | Problem | Imp. | Ease | Glue | Launch | Why |
|---|---|---|---|---|---|---|
...
| **5** | **Jobs closed / shipped with open WIP or stale status** | 4 | 4 | 4 | **16** | Status lies in ERP; effective-status inference is core Glue; enables cleaner late + unbilled logic |
...
### Tier C — Valuable but harder / less Glue-native
...
| **10** | **Slow / obsolete inventory** | 3 | 4 | 1 | **11** | Mostly single-system ERP; ERP already reports this; low Glue story |
| **11** | **Supplier / PO late receipts** | 3 | 3 | 2 | **11** | Useful; secondary to customer delivery; medium data quality |
| **12** | **Capacity vs backlog overload** | 4 | 1 | 1 | **10** | Critical business pain; routing/capacity data often fiction — not a Glue v1 play |
...
**Sweet spot for Glue launch (job shop):** high importance + medium-high ease + **high Glue novelty** → **#1 Unbilled shipments**.
...
| **Glue job** | Job/ship ↔ invoice link; customer match; effective shipped status; days-since-ship ranking |
...
| **Glue novelty** | Low — ERP report territory |
| **Ease drag** | Missing promise dates, status lies, whose date counts |
| **When to ship** | After #1 proves glue; use effective-status + date fallbacks Glue already built |
...
**Do not launch** on capacity, MES, or labor variance — high importance, wrong feasibility for Glue v1.
...
**Thesis (job-shop Signal):** Glue is a **billing completeness / cash leakage** platform that starts in job-shop manufacturing, not a general analytics suite.
...
- "ERP has a report" does **not** kill #1 if billing is in QB — that *is* the Glue gap  
