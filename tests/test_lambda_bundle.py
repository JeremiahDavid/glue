...
from lambda_bundle import (
    ...
    _deps_asset_exclude,
    _profile_asset_hash,
    hiveflow_lambda_code,
    hiveflow_lambda_deps_code,
)
...
def test_code_asset_exclude_tracks_hiveflow_source() -> None:
    assert "!packages/hiveflow-platform/src/hiveflow/**" in CODE_ASSET_EXCLUDE
    assert "!packages/hiveflow-portal/src/hiveflow/**" in CODE_ASSET_EXCLUDE
    assert "!config.yaml" in CODE_ASSET_EXCLUDE


def test_hiveflow_lambda_code_factory_returns_asset() -> None:
    first = hiveflow_lambda_code()
    second = hiveflow_lambda_code()
    assert first is not second
    assert first.path == second.path


def test_hiveflow_lambda_deps_code_factory_returns_asset() -> None:
    first = hiveflow_lambda_deps_code()
    second = hiveflow_lambda_deps_code()
    assert first is not second
    assert first.path == second.path


def test_local_code_bundling_copies_hiveflow(tmp_path) -> None:
    output_dir = tmp_path / "bundle"
    output_dir.mkdir()
    assert LocalPythonCodeBundling().try_bundle(str(output_dir), None) is True
    assert (output_dir / "hiveflow" / "__init__.py").exists()
