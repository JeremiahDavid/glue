...
def _register_entity_resolvers() -> None:
    ...
    def _catalog(cfg: dict[str, Any]) -> list[str]:
        _bundle, entity_map = resolve_qbo_entities_from_ingest_config(cfg)
        return list(entity_map)

    def _fanout(cfg: dict[str, Any]) -> list[str]:
        _bundle, entity_map = resolve_qbo_entities_from_ingest_config(cfg)
        return sorted(entity_map)

    register_bundle_resolver("qbo", resolve_qbo_entities_from_ingest_config)
    register_catalog_entities("qbo", _catalog)
    register_fanout_entities("qbo", _fanout)
