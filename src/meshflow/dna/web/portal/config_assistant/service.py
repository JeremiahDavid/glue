    ...
    *,
    ...
    if extracted:
        ...
        if extracted.get("reporting_yaml"):
            reporting = load_reporting_pack_yaml(extracted["reporting_yaml"])
            reporting = normalize_reporting_identity(
                settings, reporting, version=next_reporting_version, status="production"
            )
            candidate = dump_yaml(reporting)
