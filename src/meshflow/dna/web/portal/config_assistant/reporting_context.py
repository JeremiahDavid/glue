...
3. ranked_table — top-N YoY ranking (auto columns: name, CY YTD, PY YTD, delta, %)
   ...
   - dim_join: { output, id_column, dim_id_column?, label_columns[], title_column }
     ...
     - dim_id_column: join key on the dimension output (usually id for out_dim_* tables)
     - label_columns: dimension fields shown in the first column (joined with " · ")
     - title_column: header for the first column
   - Example: show customer name only → label_columns: [displayName]
   - Example: name + number → label_columns: [displayName, number]
...
- "Show customer name instead of ID in top customers" → ranked_table dim_join.label_columns (not columns)
