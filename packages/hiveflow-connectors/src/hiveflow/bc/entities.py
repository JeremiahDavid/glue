...
# Header → silver line table (kept here so connectors do not import lake/silver).
_DBC_HEADER_LINE_ENTITIES: dict[str, str] = {
    "sales_quotes": "sales_quote_lines",
    "sales_orders": "sales_order_lines",
    "sales_shipments": "sales_shipment_lines",
    "sales_invoices": "sales_invoice_lines",
    "open_sales_invoices": "sales_invoice_lines",
    ...
    "purchase_credit_memos": "purchase_credit_memo_lines",
}
...
def _register_entity_resolvers() -> None:
    ...
    def _catalog(cfg: dict[str, Any]) -> list[str]:
        _bundle, specs = resolve_bc_entities_from_ingest_config(cfg)
        output_names = [spec.output_name for spec in specs]
        line_entities = {
            line
            for name in output_names
            if (line := _DBC_HEADER_LINE_ENTITIES.get(name)) is not None
        }
        return [*output_names, *sorted(line_entities)]

    def _fanout(cfg: dict[str, Any]) -> list[str]:
        _bundle, specs = resolve_bc_entities_from_ingest_config(cfg)
        return [spec.output_name for spec in specs if spec.derived_from is None]

    register_bundle_resolver("dbc", resolve_bc_entities_from_ingest_config)
    register_catalog_entities("dbc", _catalog)
    register_fanout_entities("dbc", _fanout)
