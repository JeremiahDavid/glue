...
- `src/hiveflow/dna/` (everything **except** `web/`, which lives in `hiveflow-portal`)
- Packs/schema: `src/hiveflow/dna/packs/`, `src/hiveflow/dna/schema/`
- Reporting contract: `src/hiveflow/dna/reporting.py` (not under `dna.web`)
...
- **`hiveflow.dna` must not import `hiveflow.dna.web`.** Portal depends on DNA; never the reverse.
...
- Portal views/app/Cognito (`hiveflow-portal`) unless changing a shared reporting pack field
- Connector ingest clients, `../hiveflow-business`
