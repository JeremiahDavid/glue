"""Reporting pack — portal layout contract (DNA-owned).

Boilerplate: ``dna/packs/dbc_reporting_boilerplate.yaml``.
...
@lru_cache(maxsize=1)
def _reporting_pack_validator() -> Draft202012Validator:
    schema_path = reporting_pack_schema_path()
    if not schema_path.is_file():
        raise FileNotFoundError(f"Reporting pack schema not found: {schema_path}")
    schema = json.loads(schema_path.read_text(encoding="utf-8"))
    try:
        Draft202012Validator.check_schema(schema)
    except SchemaError as exc:
        raise ValueError(f"Invalid reporting-pack.schema.json: {exc.message}") from exc
...
def validate_reporting_pack_schema(payload: dict[str, Any]) -> None:
    """Validate against reporting-pack.schema.json (Draft 2020-12)."""
    if not isinstance(payload, dict):
        raise ValueError("Reporting pack must be a mapping")

    validator = _reporting_pack_validator()
    ...
    raise ValueError("Reporting pack schema error — " + "; ".join(lines) + suffix)
...
def load_reporting_pack(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "include_chart_catalog": bool(payload.get("include_chart_catalog", False)),
