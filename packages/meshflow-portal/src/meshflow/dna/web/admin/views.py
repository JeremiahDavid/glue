...
def render_admin_login_page(
    *,
    ...
    <style>
      ...
      .admin-login-form label {{ display: grid; gap: 0.35rem; font-size: 0.9rem; }}
      .admin-login-form input {{
        padding: 0.55rem 0.7rem; border: 1px solid #cbd5e1; border-radius: 6px;
      }}
      .form-error {{ color: #b91c1c; margin-top: 0.75rem; }}
    </style>
    """
    return render_page(
...
def render_admin_dashboard(
    *,
    ...
    <style>
      ...
      .admin-job-group h2 {{ font-size: 1.05rem; margin: 0 0 0.75rem; color: #334155; }}
      ...
      .admin-job-card-head h3 {{ margin: 0; font-size: 1.05rem; }}
      .admin-job-desc {{ color: #475569; margin: 0.55rem 0; }}
      .admin-job-id, .admin-job-follow, .admin-job-meta, .admin-job-message {{
        font-size: 0.85rem; color: #64748b; margin: 0.25rem 0;
      }}
      .admin-job-flash {{ color: #0f766e; font-size: 0.9rem; }}
      ...
      .admin-job-state.is-ok {{ background: #ccfbf1; color: #0f766e; }}
      .admin-job-state.is-error {{ background: #fee2e2; color: #b91c1c; }}
      .admin-extensibility-note {{
        margin-top: 2rem; font-size: 0.9rem; color: #64748b;
