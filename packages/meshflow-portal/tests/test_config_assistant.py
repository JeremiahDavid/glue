...
def test_chat_rejects_invalid_reporting_schema_before_proposal(
    seeded_settings: DnaSettings,
) -> None:
    invalid_reporting = yaml.safe_dump(
        {
            ...
            "pages": [
                {
                    "id": "page_exec",
                    "title": "Executive",
                    "sections": [{"layout": "not_a_valid_layout"}],
                }
            ],
            "changelog": [],
        },
    ...
    def fake_invoke(
        _settings: DnaSettings,
        _system: str,
        _messages: list[dict],
        _model_id: str,
    ) -> str:
        payload = {
            "summary": "Updated reporting layout",
            "reporting_yaml": invalid_reporting,
        }
        return (
            "Updated the executive page layout.\n\n"
            f"```json\n{json.dumps(payload)}\n```"
        )

    view = run_chat_turn(
        seeded_settings,
        user_message="Change the executive page layout",
        username="admin",
        invoke_fn=fake_invoke,
    )
    assert view["meta"]["status"] == "open"
    assert not view["reporting_pending"]
    assert not view["has_changes"]
    last_message = str(view["conversation"]["messages"][-1]["content"])
    assert "Reporting changes were not proposed" in last_message
    assert "schema error" in last_message.lower()


def test_chat_accepts_valid_reporting_schema_before_proposal(
    seeded_settings: DnaSettings,
) -> None:
    reporting = load_production_reporting(seeded_settings)
    reporting["description"] = str(reporting.get("description") or "") + " (schema-checked)"
    valid_reporting = yaml.safe_dump(reporting, sort_keys=False, allow_unicode=True)
    ...
    def fake_invoke(
        ...
        payload = {
            "summary": "Updated reporting description",
            "reporting_yaml": valid_reporting,
        }
        return (
            "Updated the reporting description.\n\n"
            f"```json\n{json.dumps(payload)}\n```"
        )

    view = run_chat_turn(
        seeded_settings,
        user_message="Update reporting description",
        username="admin",
        invoke_fn=fake_invoke,
    )
    assert view["reporting_pending"]
    ...
    last_message = str(view["conversation"]["messages"][-1]["content"])
    assert "were not proposed" not in last_message
