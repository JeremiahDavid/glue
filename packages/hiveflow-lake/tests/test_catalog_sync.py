...
def test_sync_source_catalog_calls_silver_and_raw() -> None:
    settings = ConsolidateSettings(
        ...
        s3_bucket="hiveflow-poc-123-us-east-2",
        raw_prefix="raw/qbd",
    )
    with (
        patch(
            "hiveflow.catalog.glue_schema.resolve_catalog_entity_names",
            return_value=["customers", "invoices"],
        ),
        patch(
            "hiveflow.catalog.glue_schema.sync_silver_table_schema",
            side_effect=lambda _settings, entity, **_kwargs: [{"Name": "id", "Type": "string"}],
        ) as silver_sync,
        patch(
            "hiveflow.catalog.glue_schema.sync_raw_tables_for_entities",
